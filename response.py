...
def process_text(text, pa):
    ...
    if "light off" in text:
        va.audio_playback("Turn off Light")
        led_off=a.Light_off()

    if "light on" in text:
        va.audio_playback("Turn on Light")
        led_on=a.Light_on()

    """Asking to start camera"""
    if "camera" in text:
        va.audio_playback("Starting camera")
        a.Camera_control()

    """Asking to set alarm"""
    if "alarm" in text:
        a.alarm()
        va.audio_playback("The alarm was Set")

    """Asking for setting a reminder in calendar"""
